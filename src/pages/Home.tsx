...
import type { Beat, Clip, Collection, Concept, Sport } from "../types";
...
export function Home() {
  ...
  useEffect(() => {
    (async () => {
      const [sportsRes, conceptsRes, clipLinksRes, collectionsRes, collectionConceptsRes] = await Promise.all([
      ...
      for (const row of collectionConceptsRes.data ?? []) {
      ...
      {collections.length > 0 && (
        <div className="mb-10">
          <div className="mb-3 flex items-baseline justify-between">
            <h2 className="text-sm uppercase tracking-widest text-text-dim">Start here</h2>
          ...
          <div className="flex gap-3 overflow-x-auto pb-1">
            {collections.slice(0, 4).map((coll) => (
              <Link
                ...
                className="pb-card w-64 shrink-0 rounded-[var(--radius-pb)] border border-surface-border bg-surface p-4"
              >
                <p className="font-display text-lg uppercase leading-tight tracking-wide">{coll.title}</p>
                {coll.description && <p className="mt-1 text-xs text-text-dim">{coll.description}</p>}
                <p className="mt-2 text-xs text-primary">{coll.conceptCount} concepts</p>
              </Link>
            ))}
          </div>
        </div>
